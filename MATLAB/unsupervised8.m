...
% Plot the accuracy/silhouette scores over epochs
figure;
subplot(1,2,1);
...
Multiple Epochs for K-Means: The code now trains the K-Means clustering for 20 epochs to refine the clusters gradually.
Silhouette Score Monitoring: After each epoch, the Silhouette Scores for both training and validation sets are calculated and plotted, so you can track how well the clustering is improving over time.
Cluster Evaluation: The final clustering result is evaluated on the test set after training, with a visual plot showing how well the data is clustered.
Clustering Quality Over Epochs: The Silhouette Scores for both training and validation sets are plotted, helping to monitor if the model is improving over time.
